...
export async function GET() {
	try {
		...
		return NextResponse.json({ error: 'An error occurred while fetching account.' }, { status: 500 });
	}
}
export async function POST(request: Request) {
	try {
		...
		if (!email) {
			return NextResponse.json({ error: 'Email and either password or passkey are required.' }, { status: 400 });
		}

		if (!hostname) return NextResponse.json({ error: 'Please provide a hostname.' }, { status: 401 });
		...
		if (customer?.password_hash && password) {
			...
			if (!isPasskeyValid) {
				return NextResponse.json({ error: 'Invalid email or passkey.' }, { status: 401 });
			}
		}

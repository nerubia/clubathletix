...
export async function PUT(request: NextRequest) {
	...
	// C095BDNLRD2
	const response = await submitSlackRequest('conversations.list', {
...
export async function POST(request: NextRequest) {
	const { event_type, name, opponent, time, location, coaches, years } = await request.json();
	...
	console.log(
		'Results:',
		results.map((r) => (r.ok ? 'OK' : r.statusText))
	);

	return NextResponse.json({ applicableChannels, params, payload });
}

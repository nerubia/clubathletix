...
export async function POST(request: NextRequest) {
	const { id, fname, lname, street_1, street_2, city_town, state_province, postal_zip_code, email } =
		await request.json();
	...
	try {
		return NextResponse.json(
			{ id, street_1, street_2, city_town, state_province, postal_zip_code, full_name, customers },
			{ status: 200 }
		);
	} catch (error) {
		return NextResponse.json({ error }, { status: 500 });

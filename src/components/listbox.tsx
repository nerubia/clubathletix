    ...
    <Headless.Listbox {...props} multiple={false}>
      <Headless.ListboxButton
        ...
        className={clsx([
          ...
          // Focus ring
          'after:pointer-events-none after:absolute after:inset-0 after:rounded-lg after:ring-transparent after:ring-inset data-focus:after:ring-2 data-focus:after:ring-slate-500',
          // Disabled state
      ...
      >
        <Headless.ListboxSelectedOption
          ...
          className={clsx([
            ...
            dataTheme !== 'light' && 'dark:border-white/10 dark:group-data-active:border-white/20 dark:group-data-hover:border-white/20',
            // Background color
            'bg-transparent',
            dataTheme !== 'light' && 'dark:bg-white/5',
            // Invalid state
            'group-data-invalid:border-red-500 group-data-hover:group-data-invalid:border-red-500',
            dataTheme !== 'light' && 'dark:group-data-invalid:border-red-600 dark:data-hover:group-data-invalid:border-red-600',
            // Disabled state
            'group-data-disabled:border-zinc-950/20 group-data-disabled:opacity-100',
            dataTheme !== 'light' && 'dark:group-data-disabled:border-white/15 dark:group-data-disabled:bg-white/[2.5%] dark:group-data-disabled:data-hover:border-white/15',
          ])}
        />
        <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
          <svg
            className={clsx('size-5 stroke-zinc-500 group-data-disabled:stroke-zinc-600 sm:size-4',
...
export function ListboxOption<T>({
  ...
  'data-theme': dataTheme,
  ...props
}: { className?: string; children?: React.ReactNode, 'data-theme'?: string } & Omit<
...
>) {
  let sharedClasses = clsx(
    ...
    dataTheme === 'light' ? '' : 'forced-colors:*:data-[slot=icon]:text-[CanvasText] forced-colors:group-data-focus/option:*:data-[slot=icon]:text-[Canvas]',
    // Avatars
        ...
        return (
          <div
            className={clsx(
              ...
              // Forced colors mode
              dataTheme !== 'light' && 'forced-color-adjust-none forced-colors:data-focus:bg-[Highlight] forced-colors:data-focus:text-[HighlightText]',
              // Disabled
...
export function ListboxDescription({ className, children, ...props }: React.ComponentPropsWithoutRef<'span'> & { 'data-theme'?: string }) {
  return (
    <span
      {...props}
      className={clsx(
